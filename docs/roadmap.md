<!-- Generated by /plan-roadmap on 2026-07-11; refreshed 2026-07-12 (M1-M3
     delivered, M4 elaborated). Advice, not pipeline state — reorder freely;
...
- Subject scheme is `bw.evt.<type>.<workflowID>` where **type may contain dots**,
  so `workflowID` is **not at a fixed subject depth** — a constraint for F4.4.
...
- [ ] **F4.4 workflowID subscription scoping** · internal/bus (+ likely /refactor-architecture) · size M · depends on: F4.1
  > **FLAG — likely a refactor, not a plain chunk.** Broker-side scoping to one `workflowID` needs a subject wildcard pinning the workflowID token, but the current scheme `bw.evt.<type>.<workflowID>` puts workflowID **last, after a variable-length (dotted) type**, so it is not at a fixed subject depth and no NATS wildcard can match it. Options: **(a)** reorder to `bw.evt.<workflowID>.<type…>` so workflowID is a fixed token — this changes the ADR-0004 subject encoding in `event`/`bus`, so route it through **/refactor-architecture** (it would become refactor-0003); or **(b)** keep in-process filtering by the event's `WorkflowID` (no broker-side benefit — the instance still receives all events). Decide the approach BEFORE coding; do not implement scheme changes by stealth.
