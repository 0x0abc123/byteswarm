<!-- Generated by /plan-roadmap on 2026-07-11; refreshed 2026-07-12 (M1+M2
     delivered, M3 elaborated). Advice, not pipeline state — reorder freely;
...
Current state: **M1 + M2 delivered.** An event flows end-to-end (`byteswarmctl
publish` → `POST /events` → NATS → `consumer.Registry` → consumers), and
config-declared JavaScript plugins run on the goja host, persisting namespaced
state in SQLite or PostgreSQL. M3 adds the authenticated webhook ingress so
external systems can trigger events. (refactor-0002 / ADR-0009: consumer state
is opaque bytes — Postgres BYTEA / SQLite BLOB — JSONB deferred.)

What M1/M2 established (reuse in M3):
- `server.New(logger, pub event.Publisher)` — the HTTP adapter, with a
  correlation-ID + structured-logging middleware chain and `POST /events` that
  bounds/validates input and publishes via the `event.Publisher` port.
- `event.Publisher` = the bus (`*bus.JetStreamBus`); events are validated at the
  ingress boundary before publish.
- `internal/auth` **Authenticator port already exists**:
  `Authenticate(ctx, credential string) error`, deny-by-default / fail-closed,
  returning `auth.ErrUnauthenticated`. No shared-secret adapter yet.
- `loadConfig(BYTESWARM_CONFIG)` (ADR-0006): committable JSON + env overrides;
  secrets come from env, never the committed file.
- Ports-and-adapters throughout: domain packages define ports; `server`/`store`/
  `bus` are adapters; wiring lives only in `cmd/byteswarm`.
...
- [ ] **F3.2 Authenticated webhook ingress endpoint** · internal/server · size M · depends on: F3.1
